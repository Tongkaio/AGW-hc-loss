...
class BasicBlock(nn.Module):  # 层数少的采用BasicBlock双层结构
  expansion = 1
...
class Bottleneck(nn.Module):  # ResNet50及以上采用Bottleneck结构,三层
  expansion = 4
...
class ResNet(nn.Module):
  ...
  def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
    ...
    for i in range(1, blocks):  # blocks 依次等于 3 4 6 3
...
def resnet50(pretrained=False, **kwargs):
  ...
  if pretrained:
    ...
    model.load_state_dict(remove_fc(model_zoo.load_url(model_urls['resnet50'])))  # 去掉预训练模型的全连接层，导入预训练模型
  return model
