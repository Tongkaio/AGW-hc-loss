...
from visualization_one_pic_gui_new import predict
...
class Camshow(QMainWindow, Ui_ReidSystem):
    ...
    def loadImage(self):
        self.fname, _ = QFileDialog.getOpenFileNames(self, '选择图片', r'D:\pythoncode\NWPU-ReID\ori_data',
                                                     '图像文件(*.jpg *.jpeg *.png)')
        if self.fname:
        ...
        def run():
            if self.mode.currentText() == '可见光搜索红外':
                ...
                target_gallery_path, labeltext, similartiytext = predict(*self.fname,
                                                                         resume='nwpu_agw_p4_n8_lr_0.1_seed_0_drop_0.5_whc_0.4_best.t',
                                                                         mode=mode)
            if self.mode.currentText() == '红外搜索可见光':
                ...
                self.ms.browse_print.emit(self.command, '开始处理，请等待......')
                target_gallery_path, labeltext, similartiytext = predict(*self.fname,
                                                                         resume='nwpu_agw_p4_n8_lr_0.1_seed_0_drop_0.5_whc_0.2_best.t',
                                                                         mode=mode)
            # --------------------------------------------------------------------------------------------
