...
def process_muti_query_sysu(pic_path):
    query_img = []
    query_id = []
    query_cam = []
...
def visualization(distmat, q_pids, g_pids, q_camids, g_camids, g_imgs, q_img):  # 相似度
    ...
    visualization_save = r'D:\pythoncode\AGW-hc-loss\visualization_save2'
    ...
    for query_path in q_img:
        ...
        plt.title(q_pids[j])
        plt.imshow(img)
        shutil.copy(query_path, os.path.join(visualization_save, str(count)+'-'+str(q_pids[j])+'.jpg'))
        count = count + 1
        plt.xticks([])
        plt.yticks([])
        for pos, label, similartiy in zip(indices[j][0:10], pred_label[j][0:10], -distmat[j][indices[j]]):
            ...
            plt.imshow(img)
            shutil.copy(path, os.path.join(visualization_save, str(count)+'-'+labelwithsimilar+'.jpg'))
            count = count + 1
            plt.xticks([])
            plt.yticks([])
        j = j + 1
...
if dataset == 'sysu':
    ...
    # testing set
    pic_path = [r'D:\pythoncode\SYSU-MM01\ori_data\cam3\0024\0015.jpg',
                r'D:\pythoncode\SYSU-MM01\ori_data\cam3\0010\0014.jpg',
                r'D:\pythoncode\SYSU-MM01\ori_data\cam3\0034\0010.jpg',
                r'D:\pythoncode\SYSU-MM01\ori_data\cam6\0021\0020.jpg']

    query_img, query_label, query_cam = process_muti_query_sysu(pic_path)
    ...
    gall_feat_fc = np.load('gall_feat_fc_t2v_sysu.npy')
...
#  python visualization_muti_pic_sysu_t2v.py --resume sysu_agw_p4_n8_lr_0.1_seed_0_drop_0.5_whc_1.2_best.t --dataset sysu --model_path thermal2visible/