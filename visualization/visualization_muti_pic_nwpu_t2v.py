...
parser.add_argument('--model_path', default='thermal2visible/', type=str,
...
def process_muti_query_nwpu(pic_path):
    query_img = []
    query_id = []
    query_cam = []
...
def visualization(distmat, q_pids, g_pids, q_camids, g_camids, g_imgs, q_img):  # 相似度
    ...
    visualization_save = r'D:\pythoncode\AGW-hc-loss\visualization_save_nwpu_t2v'
    ...
    for query_path in q_img:
        ...
        plt.title(q_pids[j])
        plt.imshow(img)
        shutil.copy(query_path, os.path.join(visualization_save, str(count)+'-'+str(q_pids[j])+'.png'))
        count = count + 1
        plt.xticks([])
        plt.yticks([])
        for pos, label, similartiy in zip(indices[j][0:10], pred_label[j][0:10], -distmat[j][indices[j]]):
            ...
            plt.imshow(img)
            shutil.copy(path, os.path.join(visualization_save, str(count)+'-'+labelwithsimilar+'.png'))
            count = count + 1
            plt.xticks([])
            plt.yticks([])
        j = j + 1
...
if dataset == 'nwpu':
    ...
    # testing set
    pic_path = [r'D:\pythoncode\NWPU-ReID\ori_data\cam5\0026\0026_2022_03_13_10_01_45_5_0_1_193.png',
                r'D:\pythoncode\NWPU-ReID\ori_data\cam4\0027\0027_2022_03_13_10_01_42_4_0_0_213.png',
                r'D:\pythoncode\NWPU-ReID\ori_data\cam1\0033\0033_2022_03_13_10_01_35_1_0_0_451.png',
                r'D:\pythoncode\NWPU-ReID\ori_data\cam1\0040\0040_2022_03_13_10_06_35_1_0_0_243.png']

    query_img, query_label, query_cam = process_muti_query_nwpu(pic_path)
    ...
    gall_feat_fc = np.load('gall_feat_fc_t2v_nwpu.npy')
...
# python visualization_muti_pic_nwpu_t2v.py --resume nwpu_agw_p4_n8_lr_0.1_seed_0_drop_0.5_whc_0.2_best.t --dataset nwpu