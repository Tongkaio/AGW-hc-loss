...
import os
# 数据集预处理
data_path = 'SYSU-MM01/ori_data'  # 未处理的数据集路径

rgb_cameras = ['cam1', 'cam2', 'cam4', 'cam5']  # 这些是RGB摄像机
ir_cameras = ['cam3', 'cam6']  # 这些是红外摄像机

# load id info
file_path_train = os.path.join(data_path, 'exp/train_id.txt')  # 这个文本里放着作为训练集的行人ID
file_path_val = os.path.join(data_path, 'exp/val_id.txt')  # 这个文本里 放着作为验证集的行人ID
...
with open(file_path_val, 'r') as file:  # 打开验证集ID的txt 处理方式是和训练集一样的
...
id_train.extend(id_val)   # 把，验证集的ID，拼接到，训练集ID，的后面了
...
for img_path in files_ir:
    pid = int(img_path[-13:-9])  # 获取行人编号，形如: 1 ，int类型的
