...
DEV_IP = create_string_buffer(b'192.168.1.104')  # 网络摄像头的IP地址
...
def CaptureVideo():
    ...
    while True:
        ...
        path = os.path.join(videoname_root, "cam" + str(lUserId) + "_" + "day_indoor_" + time_now + ".mp4")  # 文件路径指针，包括文件名
        print('文件保存路径:%s' % path)
...
if __name__ == '__main__':
    ...
    ##############################################################################################
    # 开启预览
    # lRealPlayHandle = OpenPreview(Objdll, lUserId, funcRealDataCallBack_V30)  # int类型的返回值
    ...
    video_thread.setDaemon(True)  # 主线程结束后，杀死此线程
    video_thread.start()
    # video_thread.join()
