...
DEV_IP = create_string_buffer(b'192.168.1.64')  # 网络摄像头的IP地址
...
def CaptureVideo():
    ...
    while True:
        ...
        if Objdll.NET_DVR_SaveRealData(lRealPlayHandle, bytes(path, 'utf-8')):  # 开启一次录像
            ...
            print("第%d段录像已开始 %s" % (video_count, start_time))
        else:
            print("录像失败")
        ...
        print('第%d段录像已保存 %s' % (video_count, end_time))
        print('第%d段录像文件名:%s' % (video_count, v_path))
        print('---------------------------------------------------------')
